...
class OllamaConfig(BaseModel):
    ...
    llm_log_format: str = Field(
        default="json", env="LLM_LOG_FORMAT"
    )  # "json" or "text"
    ...
    def validate_model_name(cls, v):
        ...
        if not re.match(r"^[a-zA-Z0-9][a-zA-Z0-9_\-:.]*$", v):
            raise ValueError(
                "Model name contains invalid characters. Use only alphanumeric, underscore, dash, colon, and dot."
            )
        return v
    ...
    def validate_model_phases(cls, v):
        ...
        if invalid_phases:
            raise ValueError(
                f"Invalid phases in model_map: {invalid_phases}. Valid phases are: {valid_phases}"
            )
        return v
...
"""
    ...
    def get_execution_system_prompt(self, hybrid_search_enabled: bool = False) -> str:
        ...
        return self.execution_system_prompt.replace(
            "{{HYBRID_SEARCH_SECTION}}", hybrid_section
        ).replace("{{FUNCTION_CALL_BEST_PRACTICES}}", self.FUNCTION_CALL_BEST_PRACTICES)
...
class GoogleConfig(BaseModel):
    ...
    model: str = Field(
        default="gemini-3-flash", description="Default Gemini model", env="GOOGLE_MODEL"
    )
    # Embedding model
    embedding_model: str = Field(
    ...
    model_map: Dict[str, str] = Field(
        default_factory=lambda: {"planning": "", "execution": "", "analysis": ""}
    )
    ...
    context_budget: int = Field(
        default=80000, ge=4000, le=2000000, env="CONTEXT_BUDGET"
    )
    context_budget_execution: float = Field(
        default=0.5, ge=0.1, le=0.8, env="CONTEXT_BUDGET_EXECUTION"
    )
    enable_result_summarization: bool = Field(
        default=True, env="ENABLE_RESULT_SUMMARIZATION"
    )
...
class ExternalConfig(BaseModel):
    ...
    base_url: str = Field(
        default="", description="Base URL for API", env="EXTERNAL_BASE_URL"
    )
    model: str = Field(
    ...
    embedding_model: str = Field(
        default="", description="Embedding model name", env="EXTERNAL_EMBEDDING_MODEL"
    )
    timeout: int = Field(
        ge=1,
        le=600,
    ...
    # Context Budget (reused logic)
    context_budget: int = Field(
        default=20000, ge=4000, le=2000000, env="CONTEXT_BUDGET"
    )
    context_budget_execution: float = Field(
        default=0.5, ge=0.1, le=0.8, env="CONTEXT_BUDGET_EXECUTION"
    )
    ...
    current_loop_max_chars: int = Field(
        default=2000, ge=100, le=50000, env="CURRENT_LOOP_MAX_CHARS"
    )

    prev_loop_max_chars: int = Field(
        default=800, ge=50, le=10000, env="PREV_LOOP_MAX_CHARS"
    )
    older_loop_max_chars: int = Field(
        default=200, ge=20, le=2000, env="OLDER_LOOP_MAX_CHARS"
    )
    ...
    # Orchestrator settings (sub-agent architecture)
    orchestrator_max_cycles: int = Field(
        default=15, ge=1, le=50, env="ORCHESTRATOR_MAX_CYCLES"
    )
    worker_default_max_steps: int = Field(
        default=20, ge=1, le=50, env="WORKER_DEFAULT_MAX_STEPS"
    )
    coverage_stall_threshold: int = Field(
        default=3, ge=2, le=10, env="COVERAGE_STALL_THRESHOLD"
    )
    orchestrator_doom_loop_threshold: int = Field(
        default=2, ge=2, le=5, env="ORCHESTRATOR_DOOM_LOOP_THRESHOLD"
    )
    ...
    FUNCTION_CALL_BEST_PRACTICES: ClassVar[str] = (
        OllamaConfig.FUNCTION_CALL_BEST_PRACTICES
    )
...
class CustomAPIConfig(BaseModel):
    ...
    api_url: AnyHttpUrl = Field(
        default="https://api.example.com/v1/chat/completions", env="CUSTOM_API_URL"
    )
    ...
    temperature: float = Field(
        default=0.7, ge=0.0, le=2.0, env="CUSTOM_API_TEMPERATURE"
    )
    ...
    llm_log_file: str = Field(
        default="logs/llm_interactions_custom.log", env="CUSTOM_API_LOG_FILE"
    )
    llm_log_prompts: bool = Field(default=True, env="LLM_LOG_PROMPTS")
    llm_log_responses: bool = Field(default=True, env="LLM_LOG_RESPONSES")
    llm_log_tokens: bool = Field(default=True, env="LLM_LOG_TOKENS")
    ...
    global_min_interval: float = Field(
        default=0.0, ge=0.0, env="CUSTOM_API_GLOBAL_MIN_INTERVAL"
    )
    respect_retry_after: bool = Field(
        default=True, env="CUSTOM_API_RESPECT_RETRY_AFTER"
    )
    retry_after_max_seconds: int = Field(
        default=60, ge=0, env="CUSTOM_API_RETRY_AFTER_MAX_SECONDS"
    )

    # Adaptive throttling (advanced)
    adaptive_throttle_enabled: bool = Field(
        default=True, env="CUSTOM_API_ADAPTIVE_THROTTLE_ENABLED"
    )
    adaptive_max_interval: float = Field(
        default=10.0, ge=0.0, env="CUSTOM_API_ADAPTIVE_MAX_INTERVAL"
    )
    adaptive_increase_factor: float = Field(
        default=1.5, ge=1.0, env="CUSTOM_API_ADAPTIVE_INCREASE_FACTOR"
    )
    adaptive_decrease_factor: float = Field(
        default=0.9, gt=0.0, le=1.0, env="CUSTOM_API_ADAPTIVE_DECREASE_FACTOR"
    )
    adaptive_success_streak_threshold: int = Field(
        default=10, ge=1, env="CUSTOM_API_ADAPTIVE_SUCCESS_STREAK_THRESHOLD"
    )
    adaptive_jitter_seconds: float = Field(
        default=0.25, ge=0.0, env="CUSTOM_API_ADAPTIVE_JITTER_SECONDS"
    )
...
class SessionHistoryConfig(BaseModel):
    ...
    vector_db_path: str = Field(
        default="data/vector_db", description="Path to vector database directory"
    )
...
class BridgeConfig(BaseSettings):
    ...
    context_limit: int = Field(
        ge=1, le=50, default=25, description="Context limit for conversations (1-50)"
    )
    max_steps: int = Field(
        ge=1, le=100, default=5, description="Maximum steps for task execution (1-100)"
    )

    # CAG Configuration
    cag_enabled: bool = True
    cag_knowledge_cache_enabled: bool = True
    cag_token_limit: int = Field(
        ge=100, le=50000, default=2000, description="CAG token limit (100-50000)"
    )
    ...
    knowledge_base_dir: str = Field(
        default="knowledge_base", description="Knowledge base directory path"
    )
...
def get_config() -> BridgeConfig:
    ...
    if _config_instance is None:
        ...
        if os.getenv("LLM_LOGGING_ENABLED"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["llm_logging_enabled"] = (
                os.getenv("LLM_LOGGING_ENABLED").lower() == "true"
            )
        ...
        if os.getenv("LLM_LOG_PROMPTS"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["llm_log_prompts"] = (
                os.getenv("LLM_LOG_PROMPTS").lower() == "true"
            )
        if os.getenv("LLM_LOG_RESPONSES"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["llm_log_responses"] = (
                os.getenv("LLM_LOG_RESPONSES").lower() == "true"
            )
        if os.getenv("LLM_LOG_TOKENS"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["llm_log_tokens"] = (
                os.getenv("LLM_LOG_TOKENS").lower() == "true"
            )
        if os.getenv("LLM_LOG_TIMING"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["llm_log_timing"] = (
                os.getenv("LLM_LOG_TIMING").lower() == "true"
            )
        ...
        if os.getenv("MAX_EXECUTION_STEPS"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            try:
                config_data["ollama"]["max_execution_steps"] = int(
                    os.getenv("MAX_EXECUTION_STEPS")
                )
            except ValueError:
        ...
        if os.getenv("EXECUTION_LOOP_ENABLED"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["execution_loop_enabled"] = (
                os.getenv("EXECUTION_LOOP_ENABLED").lower() == "true"
            )
        ...
        _orch_env_map = {
            ...
            "ORCHESTRATOR_DOOM_LOOP_THRESHOLD": (
                "orchestrator_doom_loop_threshold",
                int,
            ),
        }
        ...
        if os.getenv("MAX_EXECUTION_STEPS"):
            ...
            try:
                config_data["external"]["max_execution_steps"] = int(
                    os.getenv("MAX_EXECUTION_STEPS")
                )
            except ValueError:
                pass
        ...
        if os.getenv("OLLAMA_REQUEST_DELAY"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            try:
                config_data["ollama"]["request_delay"] = float(
                    os.getenv("OLLAMA_REQUEST_DELAY")
                )
            except ValueError:
                pass  # Use default if invalid value
        ...
        if os.getenv("OLLAMA_EMBEDDING_MODEL"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["embedding_model"] = os.getenv(
        ...
        if os.getenv("OLLAMA_MAX_RETRIES"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            try:
                config_data["ollama"]["max_retries"] = int(
        ...
        if os.getenv("OLLAMA_SHOW_REASONING"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["show_reasoning"] = (
                os.getenv("OLLAMA_SHOW_REASONING").lower() == "true"
            )
        ...
        if os.getenv("CONTEXT_BUDGET"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            try:
                config_data["ollama"]["context_budget"] = int(
                    os.getenv("CONTEXT_BUDGET")
                )
            except ValueError:
                pass  # Use default if invalid value
        ...
        if os.getenv("CONTEXT_BUDGET_EXECUTION"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            try:
                config_data["ollama"]["context_budget_execution"] = float(
                    os.getenv("CONTEXT_BUDGET_EXECUTION")
                )
            except ValueError:
                pass  # Use default if invalid value
        ...
        if os.getenv("ENABLE_RESULT_SUMMARIZATION"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["enable_result_summarization"] = (
                os.getenv("ENABLE_RESULT_SUMMARIZATION").lower() == "true"
            )

        if os.getenv("RESULT_CACHE_ENABLED"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["result_cache_enabled"] = (
                os.getenv("RESULT_CACHE_ENABLED").lower() == "true"
            )

        if os.getenv("TIERED_CONTEXT_ENABLED"):
            if "ollama" not in config_data:
                config_data["ollama"] = {}
            config_data["ollama"]["tiered_context_enabled"] = (
                os.getenv("TIERED_CONTEXT_ENABLED").lower() == "true"
            )
        ...
        if os.getenv("GHIDRA_MOCK_MODE"):
            if "ghidra" not in config_data:
                config_data["ghidra"] = {}
            config_data["ghidra"]["mock_mode"] = (
                os.getenv("GHIDRA_MOCK_MODE").lower() == "true"
            )
        ...
        if os.getenv("GOOGLE_EMBEDDING_MODEL"):
            if "google" not in config_data:
                config_data["google"] = {}
            config_data["google"]["embedding_model"] = os.getenv(
        ...
        if os.getenv("GOOGLE_REQUEST_DELAY"):
            if "google" not in config_data:
                config_data["google"] = {}
            try:
                config_data["google"]["request_delay"] = float(
                    os.getenv("GOOGLE_REQUEST_DELAY")
                )
            except ValueError:
                pass
        ...
        if os.getenv("GOOGLE_MAX_RETRIES"):
            if "google" not in config_data:
                config_data["google"] = {}
            try:
                config_data["google"]["max_retries"] = int(
                    os.getenv("GOOGLE_MAX_RETRIES")
                )
            except ValueError:
                pass
        ...
        if os.getenv("EXTERNAL_EMBEDDING_MODEL"):
            config_data["external"]["embedding_model"] = os.getenv(
        ...
        if os.getenv("EXTERNAL_TEMPERATURE"):
            try:
                config_data["external"]["temperature"] = float(
                    os.getenv("EXTERNAL_TEMPERATURE")
                )
            except ValueError:
                pass
        if os.getenv("EXTERNAL_MAX_TOKENS"):
            try:
                config_data["external"]["max_tokens"] = int(
                    os.getenv("EXTERNAL_MAX_TOKENS")
                )
            except ValueError:
                pass
        ...
        if os.getenv("EXTERNAL_REQUEST_DELAY"):
            try:
                config_data["external"]["request_delay"] = float(
                    os.getenv("EXTERNAL_REQUEST_DELAY")
                )
            except ValueError:
                pass
        ...
        if os.getenv("EXTERNAL_MAX_RETRIES"):
            try:
                config_data["external"]["max_retries"] = int(
                    os.getenv("EXTERNAL_MAX_RETRIES")
                )
            except ValueError:
                pass
        ...
        if os.getenv("CONTEXT_BUDGET"):
            try:
                config_data["external"]["context_budget"] = int(
                    os.getenv("CONTEXT_BUDGET")
                )
            except ValueError:
                pass
        ...
        # Logging settings
        if os.getenv("LLM_LOGGING_ENABLED"):
            config_data["external"]["llm_logging_enabled"] = (
                os.getenv("LLM_LOGGING_ENABLED").lower() == "true"
            )
        if os.getenv("LLM_LOG_FILE"):
        ...
        if os.getenv("CUSTOM_API_EMBEDDING_MODEL"):
            config_data["custom_api"]["embedding_model"] = os.getenv(
                "CUSTOM_API_EMBEDDING_MODEL"
            )
        if os.getenv("CUSTOM_API_TIMEOUT"):
            try:
                config_data["custom_api"]["timeout"] = int(
                    os.getenv("CUSTOM_API_TIMEOUT")
                )
            except ValueError:
                pass
        if os.getenv("CUSTOM_API_TEMPERATURE"):
            try:
                config_data["custom_api"]["temperature"] = float(
                    os.getenv("CUSTOM_API_TEMPERATURE")
                )
            except ValueError:
                pass
        if os.getenv("CUSTOM_API_MAX_TOKENS"):
            try:
                config_data["custom_api"]["max_tokens"] = int(
                    os.getenv("CUSTOM_API_MAX_TOKENS")
                )
            except ValueError:
                pass
        if os.getenv("CUSTOM_API_VERIFY_SSL"):
            config_data["custom_api"]["verify_ssl"] = (
                os.getenv("CUSTOM_API_VERIFY_SSL").lower() == "true"
            )
        if os.getenv("CUSTOM_API_REQUEST_DELAY"):
            try:
                config_data["custom_api"]["request_delay"] = float(
                    os.getenv("CUSTOM_API_REQUEST_DELAY")
                )
            except ValueError:
                pass
        if os.getenv("CUSTOM_API_MAX_RETRIES"):
            try:
                config_data["custom_api"]["max_retries"] = int(
                    os.getenv("CUSTOM_API_MAX_RETRIES")
                )
            except ValueError:
                pass
        ...
        if os.getenv("CUSTOM_API_MAX_CONCURRENCY"):
            try:
                config_data["custom_api"]["max_concurrency"] = int(
                    os.getenv("CUSTOM_API_MAX_CONCURRENCY")
                )
            except ValueError:
                pass
        if os.getenv("CUSTOM_API_GLOBAL_MIN_INTERVAL"):
            try:
                config_data["custom_api"]["global_min_interval"] = float(
                    os.getenv("CUSTOM_API_GLOBAL_MIN_INTERVAL")
                )
            except ValueError:
                pass
        if os.getenv("CUSTOM_API_RESPECT_RETRY_AFTER"):
            config_data["custom_api"]["respect_retry_after"] = (
                os.getenv("CUSTOM_API_RESPECT_RETRY_AFTER").lower() == "true"
            )
        if os.getenv("CUSTOM_API_RETRY_AFTER_MAX_SECONDS"):
            try:
                config_data["custom_api"]["retry_after_max_seconds"] = int(
                    os.getenv("CUSTOM_API_RETRY_AFTER_MAX_SECONDS")
                )
            except ValueError:
                pass
        ...
        if os.getenv("CUSTOM_API_ADAPTIVE_MAX_INTERVAL"):
            try:
                config_data["custom_api"]["adaptive_max_interval"] = float(
                    os.getenv("CUSTOM_API_ADAPTIVE_MAX_INTERVAL")
                )
            except ValueError:
                pass
        if os.getenv("CUSTOM_API_ADAPTIVE_INCREASE_FACTOR"):
            try:
                config_data["custom_api"]["adaptive_increase_factor"] = float(
                    os.getenv("CUSTOM_API_ADAPTIVE_INCREASE_FACTOR")
                )
            except ValueError:
                pass
        if os.getenv("CUSTOM_API_ADAPTIVE_DECREASE_FACTOR"):
            try:
                config_data["custom_api"]["adaptive_decrease_factor"] = float(
                    os.getenv("CUSTOM_API_ADAPTIVE_DECREASE_FACTOR")
                )
            except ValueError:
                pass
        ...
        if os.getenv("CUSTOM_API_ADAPTIVE_JITTER_SECONDS"):
            try:
                config_data["custom_api"]["adaptive_jitter_seconds"] = float(
                    os.getenv("CUSTOM_API_ADAPTIVE_JITTER_SECONDS")
                )
            except ValueError:
                pass
        ...
        if os.getenv("CONTEXT_BUDGET"):
            try:
                config_data["custom_api"]["context_budget"] = int(
                    os.getenv("CONTEXT_BUDGET")
                )
            except ValueError:
                pass
        if os.getenv("MAX_EXECUTION_STEPS"):
            try:
                config_data["custom_api"]["max_execution_steps"] = int(
                    os.getenv("MAX_EXECUTION_STEPS")
                )
            except ValueError:
                pass
        # Logging settings
        if os.getenv("LLM_LOGGING_ENABLED"):
            config_data["custom_api"]["llm_logging_enabled"] = (
                os.getenv("LLM_LOGGING_ENABLED").lower() == "true"
            )
        if os.getenv("LLM_LOG_FILE"):
