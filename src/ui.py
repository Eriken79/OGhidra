    ...
    def _render(self, state):
        ...
        if state.active:
            self.summary_label.config(
                text=f"Cycle {state.cycle}/{state.max_cycles}, areas {area_pct}%",
            )
        else:
            self.summary_label.config(
                text=f"Done ({state.exit_reason}, areas {area_pct}%)",
            )
        ...
        for w in state.workers:
            ...
            text = (
                f'Worker #{w.worker_number}: {prefix}"{goal_short}" {icon} ({detail})'
            )
        ...
        def load_vectors_worker():
            ...
            try:
                ...
                self.vector_status_label.config(
                    text=f"Vectors: {vectors_loaded} loaded", foreground="green"
                )
        ...
        if enabled:
            if has_summaries:
                self.grep_status.config(
                    text=f"On - {summary_count} function(s) indexed", foreground="green"
                )
            else:
                self.grep_status.config(
                    text="On - Waiting for summaries", foreground="orange"
                )
        else:
            if has_summaries:
                self.grep_status.config(
    ...
    def _set_tool_running(self, running: bool, tool_name: str = ""):
        ...
        if running:
            ...
            self.status_label.config(
                text=f"Running {tool_name}...", foreground="orange"
            )
            self.progress.start()
        else:
            self.status_label.config(text="Ready", foreground="green")
                ...
                if ai_response and ai_response.strip():
                    ...
                    if enumeration_mode in ["full_enumeration", "smart_enumeration"]:
                        ...
                        if suggested_name and is_generic_name:
                            # Perform rename using address-based tool
                            try:
                                rename_result = self.bridge.execute_command(
                                    "rename_function_by_address",
                                    {
    ...
    def _health_check(self):
        ...
        def check():
            ...
            try:
                ...
                results.append(
                    f"Ollama API: {'OK ✅' if ollama_health else 'NOT OK ❌'}"
                )
            except Exception as e:
            ...
            try:
                cag_enabled = getattr(self.bridge, "enable_cag", False)
                results.append(
                    f"CAG System: {'Enabled ✅' if cag_enabled else 'Disabled ❌'}"
                )
            except Exception as e:
