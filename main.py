    ...
    while True:
        try:
            ...
            if user_input.lower() in ("exit", "quit"):  # Allow 'quit' as well
                ...
                ollama_health = (
                    bridge.ollama.check_health() if hasattr(bridge, "ollama") else False
                )
                ghidra_health = (
                    bridge.ghidra.check_health() if hasattr(bridge, "ghidra") else False
                )

                print("\n=== Health Check ===")
                print(f"Ollama API: {'OK' if ollama_health else 'NOT OK'}")
                print(f"{ghidra_backend_label} API: {'OK' if ghidra_health else 'NOT OK'}")
                print("====================\n")
                ...
                if bridge.enable_cag and bridge.cag_manager:
                    ...
                    try:
                        vector_store_enabled = (
                            config.session_history.use_vector_embeddings
                            if hasattr(config, "session_history")
                            else False
                        )
                        print(
                            f"Vector embeddings: {'Enabled ✅' if vector_store_enabled else 'Disabled ❌'}"
                        )

                        if (
                            vector_store_enabled
                            and hasattr(bridge, "memory_manager")
                            and bridge.memory_manager is not None
                        ):
                            ...
                            if mm.vector_store:
                                ...
                                print(
                                    f"Vectors available: {'Yes ✅' if vector_count > 0 else 'No ❌'}"
                                )
                                print(f"Vector count: {vector_count}")

                                if vector_count > 0:
                                    print(
                                        f"Vector dimension: {mm.vector_store.vectors.shape[1]}"
                                    )
                                    # Calculate mean norm
                                    import numpy as np

                                    norms = np.linalg.norm(
                                        mm.vector_store.vectors, axis=1
                                    )
                                    print(
                                        f"Mean vector norm: {float(np.mean(norms)):.4f}"
                                    )
                ...
                models = (
                    bridge.ollama.list_models() if hasattr(bridge, "ollama") else []
                )
                ...
                try:
                    ...
                    print(
                        f"Found {len(tools)} available tools (via run-tool command):\n"
                    )
                ...
                if bridge.enable_cag and bridge.cag_manager:
                    ...
                    print(
                        f"Knowledge Base Enabled (within CAG): {'Yes' if kb_enabled else 'No'}"
                    )
                    if kb_enabled and "vector_store" in cag_details:
                        vs_info = cag_details["vector_store"]
                        print(
                            f"  Vector Store - Function Signatures: {vs_info.get('function_signatures', 0)}"
                        )
                        print(
                            f"  Vector Store - Binary Patterns: {vs_info.get('binary_patterns', 0)}"
                        )
                        print(
                            f"  Vector Store - Analysis Rules: {vs_info.get('analysis_rules', 0)}"
                        )
                        print(
                            f"  Vector Store - Common Workflows: {vs_info.get('common_workflows', 0)}"
                        )
                    ...
                    if session_info:
                        ...
                        print(
                            f"  Tool Executions: {session_info.get('tool_executions', 0)}"
                        )
                        print(
                            f"  Decompiled Functions: {session_info.get('decompiled_functions', 0)}"
                        )
                        print(
                            f"  Renamed Entities: {session_info.get('renamed_entities', 0)}"
                        )
                        print(
                            f"  Analysis Results Cached: {session_info.get('analysis_results', 0)}"
                        )
                    else:
                    ...
                    if hasattr(
                        bridge.config, "cag_token_limit"
                    ):  # Assuming token_limit is in BridgeConfig.cag_token_limit
                        print(
                            f"CAG Token Limit (config): {bridge.config.cag_token_limit}"
                        )
                ...
                print(
                    "  health                            - Check API health and vector store status"
                )
                print("\nAnalysis Commands:")
                print(
                ...
                print(
                    "  review-session                    - Ask a query about the current session's interactions"
                )
                print("\nAdvanced Commands (hidden from prompt):")
                print(
                    "  vector-store                      - Display detailed vector store information"
                )
                print(
                    "  models                            - List available Ollama models"
                )
                print(
                    "  tools                             - List all available Ghidra tools with parameters"
                )
                print(
                    "  run-tool tool_name(p1='v1')       - Execute a specific Ghidra tool directly"
                )
                print(
                    "  cag                               - Display Context-Aware Generation status"
                )
                print(
                    "  clear_log                         - Clear the in-memory log for the current session review"
                )
                print(
                ...
                try:
                    ...
                    raw_params_str = tool_str[
                        tool_str.find("(") + 1 : tool_str.rfind(")")
                    ].strip()
                    ...
                    if raw_params_str:
                        ...
                        for pair in param_pairs:
                            if "=" in pair:
                                ...
                                if (
                                    value_str_from_pair.startswith("'")
                                    and value_str_from_pair.endswith("'")
                                ) or (
                                    value_str_from_pair.startswith('"')
                                    and value_str_from_pair.endswith('"')
                                ):
                                    final_value_for_param = (
                                        value_str_from_pair[1:-1]
                                        .encode("utf-8")
                    ...
                    if hasattr(bridge.ghidra, tool_name):
                        ...
                        bridge.logger.info(
                            f"Executing direct tool call via 'run-tool': {tool_name} with params: {params}"
                        )
                        ...
                        if tool_name in TOOLS_WITH_AI_ANALYSIS:
                            ...
                            if not is_error:
                                formatted_tool_data = ""
                                if isinstance(raw_tool_result, dict) or isinstance(
...
```"""
                                ...
                                if analysis_prompt:
                                    print(
                                        f"Sending output from {tool_name} to AI for analysis..."
                                    )
                                    try:
                                        ai_analysis = (
                                        ...
                                        bridge.logger.info(
                                            f"AI analysis received snippet: '{ai_analysis[:50]}...'"
                                        )
                                        print(
                    ...
                    bridge.logger.error(
                        f"Error executing tool '{tool_str}': {e}", exc_info=True
                    )
                    current_session_log.append(
                        f"=== Error executing tool command '{tool_str}': {e} ===\\n"
                    )

            elif user_input.lower().startswith(
                "analyze-function"
            ):  # Restored analyze-function shortcut
                try:
                    address = None
                    ...
                    current_session_log.append(
                        f"=== Result of analyze-function({params_for_log}) ===\\n{raw_tool_result}\\n"
                    )
                    ...
                    if raw_tool_result != "Ghidra client not available." and not (
                        isinstance(raw_tool_result, str)
                        and raw_tool_result.lower().startswith("error:")
                    ):
                        formatted_tool_data = ""
                        if isinstance(raw_tool_result, dict) or isinstance(
                        ...
                        print(
                            f"Sending output from analyze-function to AI for analysis..."
                        )
                        try:
                            ai_analysis = (
                        ...
                        print(
                            f"Skipping AI analysis due to tool error: {raw_tool_result}"
                        )

                except Exception as e:
                    print(f"Error analyzing function: {str(e)}")
                    bridge.logger.error(
                        f"Error in 'analyze-function' shortcut: {e}", exc_info=True
                    )
                    current_session_log.append(
                        f"=== Error in analyze-function shortcut: {e} ===\\n"
                    )
                ...
                print(
                    "4. Rename functions with generic names (FUN_, sub_, etc.) based on AI suggestions"
                )
                print(
                    "5. Load all analyzed functions into the vector store for RAG queries"
                )
                print(
                    "\nℹ️  Functions with descriptive names will be analyzed but NOT renamed."
                )
                print(
                    "⚠️  This may take considerable time depending on the number of functions."
                )
                print(
                    "    Expect ~3-8 seconds per function (includes context gathering)."
                )
                    ...
                    for i, full_function_string in enumerate(valid_functions, 1):
                        try:
                            ...
                            if " at " in full_function_string:
                                function_name = full_function_string.split(" at ")[
                                    0
                                ].strip()
                                ...
                                name_address_match = re.search(
                                    r"([0-9a-fA-F]{8,})", function_name
                                )
                                address = (
                                    name_address_match.group(1)
                            ...
                            # Decompile the function. Use address-based
                            # decompilation so this works reliably across
                            # backends (HTTP MCP and pyGhidra) regardless of
                            # how auto-generated names are formatted.
                            function_decompile_result = (
                                bridge.ghidra.decompile_function_by_address(
                                    address=address
                                )
                                if hasattr(bridge, "ghidra")
                                else None
                            ...
                            if not function_decompile_result or (
                                ...
                                and function_decompile_result.lower().startswith(
                                    "error:"
                                )
                            ):
                                print(
                            ...
                            print(
                                f"  ✓ Decompiled ({len(function_decompile_result)} chars)"
                            )
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if (
                                        isinstance(callers_result, list)
                                        and callers_result
                                    ):
                                        caller_addresses = []
                                        ...
                                        for caller_addr in caller_addresses[:3]:
                                            try:
                                                ...
                                                if (
                                                    caller_code
                                                    and not caller_code.lower().startswith(
                                                        "error"
                                                    )
                                                ):
                                ...
                                try:
                                    ...
                                    if (
                                        isinstance(callees_result, list)
                                        and callees_result
                                    ):
                                        callee_addresses = []
                                        ...
                                        for callee_addr in callee_addresses[:3]:
                                            try:
                                                ...
                                                if (
                                                    callee_code
                                                    and not callee_code.lower().startswith(
                                                        "error"
                                                    )
                                                ):
                                ...
                                context["total_chars"] = sum(
                                    len(c["code"]) for c in context["callers_code"]
                                )
                                context["total_chars"] += sum(
                                    len(c["code"]) for c in context["callees_code"]
                                )
                            ...
                            if context["callers_code"] or context["callees_code"]:
                                ...
                                if context["callees_code"]:
                                    ...
                                    for idx, callee in enumerate(
                                        context["callees_code"], 1
                                    ):
                                        sections.append(
...
CRITICAL: You MUST include all four sections with the exact headers shown above. Focus on making the suggested name as specific and descriptive as possible."""
                            ...
                            ai_response = (
                                bridge.ollama.generate(prompt=analysis_query)
                                if hasattr(bridge, "ollama")
                                else None
                            )
                            ...
                            if ai_response and ai_response.strip():
                                ...
                                for line in lines:
                                    ...
                                    if (
                                        "Suggested Name:" in line_stripped
                                        or "suggested name:" in line_stripped.lower()
                                    ):
                                        name_part = (
                                ...
                                if suggested_name and is_generic_name:
                                    ...
                                    try:
                                        ...
                                        if isinstance(
                                            rename_result, str
                                        ) and rename_result.lower().startswith(
                                            "error:"
                                        ):
                                            print(f"  ⚠ Rename failed: {rename_result}")
                                            print(
                                                f"  ℹ️  Keeping original name: {function_name}"
                                            )
                                        else:
                                            print(
                                                f"  ✅ Renamed: {function_name} → {suggested_name}"
                                            )
                                        ...
                                        bridge.logger.warning(
                                            f"Failed to rename {function_name}: {e}"
                                        )
                                    ...
                                    print(
                                        f"  ⚠ Could not extract function name from AI response"
                                    )
                                    print(
                                        f"  ℹ️  Keeping original name: {function_name}"
                                    )
                                ...
                                if hasattr(bridge, "function_summaries"):
                                    bridge.function_summaries[address] = (
                                        function_summary
                                    )
                    ...
                    if not processed_functions_data:
                        ...
                        if not (
                            ...
                            print(
                                "  ⚠ RAG system is not enabled. Vector loading skipped."
                            )
                            print(
                                "  Enable CAG in configuration to use vector operations."
                            )
                        else:
                            try:
                                vectors_loaded = 0
                                ...
                                if not test_embeddings:
                                    print(
                                        "  ⚠ Ollama embedding model (nomic-embed-text) not available."
                                    )
                                    ...
                                    BATCH_SIZE = 50
                                    num_batches = (
                                        len(processed_functions_data) + BATCH_SIZE - 1
                                    ) // BATCH_SIZE
                                    ...
                                    for batch_num in range(num_batches):
                                        ...
                                        batch = processed_functions_data[
                                        ...
                                        batch_texts = [
                                            func["summary"] for func in batch
                                        ]
                                        batch_embeddings_list = (
                                            Bridge.get_ollama_embeddings(batch_texts)
                                        )

                                        if not batch_embeddings_list:
                                            print(
                                                f"    ⚠ Failed to generate embeddings for batch {batch_num + 1}"
                                            )
                                        ...
                                        # Add to vector store
                                        for func_data, embedding in zip(
                                            ...
                                            try:
                                                ...
                                                # Add to vector store
                                                vector_store = (
                                                    bridge.cag_manager.vector_store
                                                )
                                                if vector_store:
                                                    vector_store.documents.append(
                                                        function_doc
                                                    )

                                                    # Add embedding
                                                    if isinstance(
                                                        vector_store.embeddings, list
                                                    ):
                                                        vector_store.embeddings.append(
                                                            embedding
                                                        )
                                                    else:
                                                        if (
                                                            len(vector_store.embeddings)
                                                            == 0
                                                        ):
                                    ...
                                    print(
                                        f"  ✓ Loaded {vectors_loaded} vectors into store"
                                    )
                                    if vectors_failed > 0:
                                        print(
                                            f"  ⚠ Failed to load {vectors_failed} vectors"
                                        )

                            except Exception as e:
                                print(f"  ✗ Error loading vectors: {e}")
                                bridge.logger.error(
                                    f"Error loading vectors: {e}", exc_info=True
                                )
                    ...
                    print(
                        f"  • Vectors loaded: {vectors_loaded if 'vectors_loaded' in locals() else 0}"
                    )
                    print(f"⚡ Performance:")
                    ...
                    print(
                        f"ℹ️  Functions with descriptive names were analyzed but kept their original names."
                    )
                    print(
                        f"ℹ️  All analyses are available in the vector store for enhanced queries."
                    )
                    print("=" * 60 + "\n")
                    ...
                    bridge.logger.error(
                        f"Error in enumerate-binary: {e}", exc_info=True
                    )
                    current_session_log.append(
                        f"=== Error in enumerate-binary: {e} ===\\n"
                    )
                ...
                try:
                    ...
                    bridge.logger.error(
                        f"Error during AI session review: {e}", exc_info=True
                    )
                ...
                try:
                    if hasattr(bridge, "process_query"):
                        ...
                        result = bridge.process_query(
                            user_input
                        )  # Assumes bridge has process_query
                        print("\n=== AI Agent Response ===")
                        print(result)
                        print("=========================\n")
                        current_session_log.append(
                            f"=== AI Agent Response to Query: '{user_input}' ===\\n{result}\\n"
                        )
                    else:
                        print(
                        ...
                        current_session_log.append(
                            f"=== Attempted general query (not processed): '{user_input}' ===\\n"
                        )
                    ...
                    current_session_log.append(
                        f"=== Error processing query '{user_input}': {e} ===\\n"
                    )
            ...
            bridge.logger.error(
                f"Unexpected error in interactive loop: {e}", exc_info=True
            )
...
def check_and_initialize_vector_db():
    ...
    if vectors_file.exists():
        try:
            ...
            logger.info(
                f"✅ Found existing vector database with {vectors.shape[0]} vectors"
            )
            return True
        except Exception as e:
    ...
    logger.info(
        f"🚀 Initializing vector database from {len(available_kb_files)} knowledge sources..."
    )
